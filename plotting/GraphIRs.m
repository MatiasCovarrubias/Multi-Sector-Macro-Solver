function GraphIRs(irs_1, irs_2, irs_3, ax, N, labels, range_padding, opts, legend_labels, aggregate_irs)
...
%   aggregate_irs  - (Optional) Struct with fields series_1/2/3 containing
%                    aggregate IRF blocks aligned with irs_1/2/3
    ...
    if nargin < 10 || isempty(aggregate_irs)
    ...
    for idx = 1:numel(sector_indices)
        ...
        agg1 = get_aggregate_data_or_empty(aggregate_irs, 'series_1', idx);
        agg2 = get_aggregate_data_or_empty(aggregate_irs, 'series_2', idx);
        agg3 = get_aggregate_data_or_empty(aggregate_irs, 'series_3', idx);
        ...
        plot_aggregate_panel(ax, N, d1, d2, d3, agg1, agg2, agg3, ...
            n_series, range_padding, line_specs, legend_entries, t_offset, style, R);
        ...
        if save_figures
            ...
            save_figure_png(local_fig_sec_in, fullfile(figures_folder, ['IR_IN_', prefix, sec_label_str, '.png']));
            save_figure_png(local_fig_sec_out, fullfile(figures_folder, ['IR_OUT_', prefix, sec_label_str, '.png']));
            save_figure_png(local_fig_agg, fullfile(figures_folder, ['IR_AGGR_', prefix, sec_label_str, '.png']));
            save_figure_png(local_fig_client_in, fullfile(figures_folder, ['IR_CLIENT_IN_', prefix, sec_label_str, '.png']));
            save_figure_png(local_fig_client_out, fullfile(figures_folder, ['IR_CLIENT_OUT_', prefix, sec_label_str, '.png']));
...
function data = get_aggregate_data_or_empty(aggregate_irs, field_name, idx)
    data = [];
    if ~isstruct(aggregate_irs) || ~isfield(aggregate_irs, field_name)
        return;
    end

    aggregate_cell = aggregate_irs.(field_name);
    if isempty(aggregate_cell)
        return;
    end

    data = aggregate_cell{idx};
end
...
function save_figure_png(fig_handle, filename)
    if ~ishandle(fig_handle)
        error('GraphIRs:InvalidFigureHandle', ...
            'Cannot save figure because the handle is invalid.');
    end
    ...
    if exist('exportgraphics', 'file') == 2
        exportgraphics(fig_handle, filename, 'Resolution', 150);
    else
...
function plot_subplot_series(ax, baseline, s1, s2, s3, N, range_padding, specs, t_off, style)
    hold on;

    plot(ax, baseline * ones(1, N), 'Color', style.colors.baseline, ...
        'LineWidth', style.linewidth_baseline, 'LineStyle', ':');
    ...
    if ~isempty(s1)
        plot(ax, s1(t1:t2), 'Color', specs{1}{1}, ...
            'LineWidth', specs{1}{2}, 'LineStyle', specs{1}{3});
    end
    if ~isempty(s2)
        plot(ax, s2(t1:t2), 'Color', specs{2}{1}, ...
            'LineWidth', specs{2}{2}, 'LineStyle', specs{2}{3});
    end
    if ~isempty(s3)
        plot(ax, s3(t1:t2), 'Color', specs{3}{1}, ...
            'LineWidth', specs{3}{2}, 'LineStyle', specs{3}{3});
    end
    hold off;
...
function plot_aggregate_panel(ax, N, d1, d2, d3, agg1, agg2, agg3, n_series, padding, specs, legend_entries, t_off, style, R)
    subplot(2, 4, 1);
    ...
    subplot(2, 4, 2);
    plot_subplot_series(ax, 0, ...
        get_aggregate_series(agg1, d1, 'C', R.C_exp), ...
        get_aggregate_series(agg2, d2, 'C', R.C_exp), ...
        get_aggregate_series(agg3, d3, 'C', R.C_exp), ...
        N, padding, specs, t_off, style);
    title('$C$ (Consumption Expenditure)', 'Interpreter', 'latex', 'FontSize', style.subtitle_fontsize);

    subplot(2, 4, 3);
    plot_subplot_series(ax, 0, ...
        get_aggregate_series(agg1, d1, 'L', R.L), ...
        get_aggregate_series(agg2, d2, 'L', R.L), ...
        get_aggregate_series(agg3, d3, 'L', R.L), ...
        N, padding, specs, t_off, style);
    title('$L$ (Headcount Labor)', 'Interpreter', 'latex', 'FontSize', style.subtitle_fontsize);

    subplot(2, 4, 4);
    plot_subplot_series(ax, 0, ...
        get_aggregate_series(agg1, d1, 'I', R.I_exp), ...
        get_aggregate_series(agg2, d2, 'I', R.I_exp), ...
        get_aggregate_series(agg3, d3, 'I', R.I_exp), ...
        N, padding, specs, t_off, style);
    title('$I$ (Investment Expenditure)', 'Interpreter', 'latex', 'FontSize', style.subtitle_fontsize);

    subplot(2, 4, 5);
    plot_subplot_series(ax, 0, ...
        get_aggregate_series(agg1, d1, 'GDP', R.GDP_exp), ...
        get_aggregate_series(agg2, d2, 'GDP', R.GDP_exp), ...
        get_aggregate_series(agg3, d3, 'GDP', R.GDP_exp), ...
        N, padding, specs, t_off, style);
    title('$GDP$ (Expenditure GDP)', 'Interpreter', 'latex', 'FontSize', style.subtitle_fontsize);

    subplot(2, 4, 6);
    plot_subplot_series(ax, 0, ...
        get_aggregate_series(agg1, d1, 'K', R.K), ...
        get_aggregate_series(agg2, d2, 'K', R.K), ...
        get_aggregate_series(agg3, d3, 'K', R.K), ...
        N, padding, specs, t_off, style);
    title('$K$ (Price-Weighted Capital)', 'Interpreter', 'latex', 'FontSize', style.subtitle_fontsize);

    subplot(2, 4, 7);
    plot_subplot_series(ax, 0, ...
        get_aggregate_series(agg1, d1, 'utility_intratemp', R.utility_intratemp), ...
        get_aggregate_series(agg2, d2, 'utility_intratemp', R.utility_intratemp), ...
        get_aggregate_series(agg3, d3, 'utility_intratemp', R.utility_intratemp), ...
        N, padding, specs, t_off, style);
    title('$U^{intra}$ (Intratemporal Utility)', 'Interpreter', 'latex', 'FontSize', style.subtitle_fontsize);

    subplot(2, 4, 8);
    axis off;
end

function series = get_aggregate_series(aggregate_data, ir_data, field_name, fallback_row)
    if ~isempty(aggregate_data) && isstruct(aggregate_data) && isfield(aggregate_data, field_name)
        series = aggregate_data.(field_name);
        return;
    end

    if ~isempty(ir_data) && ~isempty(fallback_row)
        series = ir_data(fallback_row, :);
        return;
    end
