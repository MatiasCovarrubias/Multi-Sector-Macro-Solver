...
@#for j in 1:n_sectors
    ...
    theta * exp(l_util)^(eps_l^(-1))*(exp(l_@{j})/exp(l_util))^(1/sigma_l) = (exp(p_@{j})) * (exp(a_@{j}))^((sigma_y-1)/sigma_y) * ( (mu_@{j}) * exp(q_@{j})/exp(y_@{j}) )^(1/sigma_q) * ( (1-alpha_@{j}) * exp(y_@{j})/exp(l_@{j}) )^(1/sigma_y);
    
    exp(pk_@{j}) = beta * (((exp(c_util(+1)) - theta*(1/(1+eps_l^(-1))) * exp(l_util(+1))^(1+eps_l^(-1)))^(-eps_c^(-1))) / MU) * (exp(p_@{j}(+1)) * exp(a_@{j}(+1))^((sigma_y-1)/sigma_y) * ((mu_@{j}) * exp(q_@{j}(+1))/exp(y_@{j}(+1)))^(1/sigma_q) * (alpha_@{j} * exp(y_@{j}(+1))/exp(k_@{j}(+1)))^(1/sigma_y) + exp(pk_@{j}(+1)) * ((1-delta_@{j}) + phi/2 * ((exp(i_@{j}(+1))/exp(k_@{j}(+1)))^2 - delta_@{j}^2)));
