...
function arres = arma_mlear( y, p, q, info )

% Code obtained from http://siteresources.worldbank.org/DEC/Resources/Hevia_arma_mle.m
% ARMA_MLE.M
...
% Edited by Christian vom Lehn. July 2017
...
% coefs1 = fminunc( @log_likelihood, 0.9*ones(size(coefs)), options );
% Override - the initial code bases guesses off of OLS estimates of the
% autocorrelation parameter, however if these are greater than 1, the
% solver gets an undefined value at the initial point.  To compensate, I
% have adjusted the initial guess to always be 0.9 (suitable for my
% applications). (CvL. 8/17)
coefs1 = fminunc( @log_likelihood, 0.9*ones(size(coefs)), options );
