...
* Winsorize sectoral annual TFP growth within sector over time
local winsor_p_low = 5
local winsor_p_high = 95

foreach var in dlog_tfp dlog_tfp_smooth dlog_tfp_go dlog_tfp_go_smooth dlog_tfp_go_noVA dlog_tfp_go_noVA_smooth {
	bysort sector: egen p`winsor_p_low'_`var' = pctile(`var'), p(`winsor_p_low')
	bysort sector: egen p`winsor_p_high'_`var' = pctile(`var'), p(`winsor_p_high')
	gen `var'_wds = `var'
	replace `var'_wds = p`winsor_p_low'_`var' if `var'_wds < p`winsor_p_low'_`var' & !missing(`var'_wds)
	replace `var'_wds = p`winsor_p_high'_`var' if `var'_wds > p`winsor_p_high'_`var' & !missing(`var'_wds)
}
...
gen TFP_GO_noVA_wds = 0
gen TFP_GO_noVA_sm_wds = 0

replace TFP = 1 if year==1948
replace TFP_sm = 1 if year==1948
replace TFP_wds = 1 if year==1948
replace TFP_sm_wds = 1 if year==1948
replace TFP_GO = 1 if year==1948
replace TFP_GO_sm = 1 if year==1948
replace TFP_GO_wds = 1 if year==1948
replace TFP_GO_sm_wds = 1 if year==1948
replace TFP_GO_noVA = 1 if year==1948
replace TFP_GO_noVA_sm = 1 if year==1948
replace TFP_GO_noVA_wds = 1 if year==1948
replace TFP_GO_noVA_sm_wds = 1 if year==1948
replace TFP = TFP[_n - 1]*exp(dlog_tfp[_n]) if year>1948
replace TFP_sm = TFP_sm[_n - 1]*exp(dlog_tfp_smooth[_n]) if year>1948
replace TFP_wds = TFP_wds[_n - 1]*exp(dlog_tfp_wds[_n]) if year>1948
replace TFP_sm_wds = TFP_sm_wds[_n - 1]*exp(dlog_tfp_smooth_wds[_n]) if year>1948
...
replace TFP_GO_wds = TFP_GO_wds[_n - 1]*exp(dlog_tfp_go_wds[_n]) if year>1948
replace TFP_GO_sm_wds = TFP_GO_sm_wds[_n - 1]*exp(dlog_tfp_go_smooth_wds[_n]) if year>1948
...
replace TFP_GO_noVA_wds = TFP_GO_noVA_wds[_n - 1]*exp(dlog_tfp_go_noVA_wds[_n]) if year>1948
replace TFP_GO_noVA_sm_wds = TFP_GO_noVA_sm_wds[_n - 1]*exp(dlog_tfp_go_noVA_smooth_wds[_n]) if year>1948
...
use bea_panel_stata_37, clear
keep year sector TFP_wds
reshape wide TFP_wds*, i(year) j(sector)
save Output\TFP_37_wds.dta, replace
export delimited using Output\TFP_37_wds.csv, replace

use bea_panel_stata_37, clear
keep year sector TFP_sm_wds
reshape wide TFP_sm_wds*, i(year) j(sector)
save Output\TFP_37_sm_wds.dta, replace
export delimited using Output\TFP_37_sm_wds.csv, replace
...
use bea_panel_stata_37, clear
keep year sector TFP_GO_wds
reshape wide TFP_GO_wds*, i(year) j(sector)
save Output\TFP_GO_37_wds.dta, replace
export delimited using Output\TFP_GO_37_wds.csv, replace

use bea_panel_stata_37, clear
keep year sector TFP_GO_sm_wds
reshape wide TFP_GO_sm_wds*, i(year) j(sector)
save Output\TFP_GO_37_sm_wds.dta, replace
export delimited using Output\TFP_GO_37_sm_wds.csv, replace
...
use bea_panel_stata_37, clear
keep year sector TFP_GO_noVA_wds
reshape wide TFP_GO_noVA_wds*, i(year) j(sector)
save Output\TFP_GO_noVA_37_wds.dta, replace
export delimited using Output\TFP_GO_noVA_37_wds.csv, replace

use bea_panel_stata_37, clear
keep year sector TFP_GO_noVA_sm_wds
reshape wide TFP_GO_noVA_sm_wds*, i(year) j(sector)
save Output\TFP_GO_noVA_37_sm_wds.dta, replace
export delimited using Output\TFP_GO_noVA_37_sm_wds.csv, replace
